...
public class MetroMenuPaneController {
  ...
  protected void initialize() {
    initIconButton(conversion, FontAwesomeIcon.GEARS, evt -> ConverterClientApplication.getInstance().openConversion());
    initIconButton(serverLog, FontAwesomeIcon.SERVER, evt -> ConverterClientApplication.getInstance().openServerLog());
    initIconButton(jmx, FontAwesomeIcon.AREA_CHART, evt -> ConverterClientApplication.getInstance().openJMX());
    initIconButton(clusterHealth, FontAwesomeIcon.HEARTBEAT, evt -> ConverterClientApplication.getInstance().openClusterHealth());
    initIconButton(inspector, FontAwesomeIcon.SEARCH, evt -> ConverterClientApplication.getInstance().openInspector());
    initSmallIconButton(options, FontAwesomeIcon.SLIDERS, evt -> ConverterClientApplication.getInstance().openOptions());
    initSmallIconButton(about, FontAwesomeIcon.INFO, evt -> ConverterClientApplication.getInstance().openAbout());
    initSmallIconButton(fullscreen, FontAwesomeIcon.EXPAND, evt -> {((Stage)pane.getScene().getWindow()).setFullScreen(true);});
    initSmallIconButton(exitFullscreen, FontAwesomeIcon.COMPRESS, evt -> {((Stage)pane.getScene().getWindow()).setFullScreen(false);});
    initSmallIconButton(update, FontAwesomeIcon.BULLHORN, evt -> {
